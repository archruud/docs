* [🏠 Hjem](/)

* 🐧 **Linux**
  * 🏹 **Arch**
    * 🖥️ **Hyprland**
      * [Skjermoppsett + Clamshell](linux/arch/hyprland/hyprland-skjermoppsett.md)
      * [Ekstra Skjerm + Waybar](linux/arch/hyprland/arch-linux-hyprland-ekstra-skjerm-waybar-guide.md)
      * [Workspace Oppsett](linux/arch/hyprland/hyprland-workspace-oppsett.md)
      * [Multi-Monitor Usermodules](linux/arch/hyprland/usermodules-archruud-multimonitor.md)
      * [Migrering: conf → Lua (0.55+)](linux/arch/hyprland/hyprland-lua-migrering.md)
      * [Tmux Dropdown](linux/arch/hyprland/tmux-dropdown.md)
      * [hdrop Guide](linux/arch/hyprland/hdrop-guide.md)
      * [Bluetooth Oppsett](linux/arch/hyprland/bluetooth-arch-hyprland.md)
      * [second user setup](linux/arch/hyprland/second-user-setup.md)
      * 📦 **Installasjon (nummerert rekkefølge)**
        * [01 — Base System Setup](linux/arch/hyprland/base-system-setup.md)
        * [02 — AWWW (Bakgrunnsbilde)](linux/arch/hyprland/awww-setup.md)
        * [03 — SDDM (Innlogging)](linux/arch/hyprland/sddm-setup.md)
        * [04 — Kitty (Terminal + Bash)](linux/arch/hyprland/kitty-setup.md)
        * [05 — Make Executable / run-alias](linux/arch/hyprland/make-executable.md)
        * [06 — Nettverk & Samba](linux/arch/hyprland/network.md)
        * [07 — Screenshots](linux/arch/hyprland/screenshots.md)
        * [08 — Quickshell-moduler](linux/arch/hyprland/quickshell/quickshell-modules-setup.md)
        * [09 — Hypridle](linux/arch/hyprland/hypridle-setup.md)
        * [10 — Hyprlock](linux/arch/hyprland/hyprlock-setup.md)
      * 🧩 **Additional Programs** (ikke del av grunninstallasjonen)
        * [KVM/QEMU Testmiljø](linux/arch/hyprland/kvm-qemu-setup.md)
        * [VM GPU-akselerasjon (Venus)](linux/arch/hyprland/vm-gpu-setup.md)
        * [NUC 16 Pro — Panther Lake Drivere](linux/arch/hyprland/nuc16-panther-lake-drivers.md)
      * 🐚 **Quickshell**
        * [Oversikt](linux/arch/hyprland/quickshell/quickshell-oversikt.md)
        * [Bar](linux/arch/hyprland/quickshell/quickshell-bar.md)
        * [Overview](linux/arch/hyprland/quickshell/quickshell-overview.md)
        * [Kalender](linux/arch/hyprland/quickshell/quickshell-kalender.md)
        * [Virtuelt tastatur (norsk)](linux/arch/hyprland/quickshell/wvkbd-norsk.md)
        * [Lag eget tastatur-layout](linux/arch/hyprland/quickshell/wvkbd-eget-layout.md)
    * [Arch Kommandoer](linux/arch/arch-kommandoer.md)
    * [Vim Guide](linux/arch/vim-guide.md)
    * [Søking i Linux](linux/arch/soking-guide.md)
    * [Tips og Aliaser](linux/arch/arch-tips-aliases.md)
    * [SCP Guide](linux/arch/scp-guide.md)
    * [Bruker og Rettigheter](linux/arch/bruker-rettigheter.md)
    * [Quick Reference](linux/arch/quick-reference.md)
    * [NordVPN på Arch](linux/arch/nordvpn-arch.md)

  * 🐧 **Debian**
    * [Debian 13: Statisk IP + Sudo](linux/debian/13/statisk-ip-sudo-oppsett.md)
    * ⚙️ **Proxmox VE 9.1.5** (Debian 13 / Trixie)
      * [GPU Drivere](linux/debian/13/proxmox/9.1.5/gpu-drivere.md)
      * [NVIDIA Oversikt](linux/debian/13/proxmox/9.1.5/nvidia-oversikt.md)
      * [NVIDIA Driver direkte i LXC](linux/debian/13/proxmox/9.1.5/nvidia-driver-direkte-i-lxc.md)
      * [NVIDIA Host Setup (for LXC)](linux/debian/13/proxmox/9.1.5/nvidia-host-setup.md)
      * [Feilsøking: Kernel-oppdatering knekker GPU](linux/debian/13/proxmox/9.1.5/kernel-gpu-feilsoking.md)
      * [ZFS Oppgradering](linux/debian/13/proxmox/9.1.5/zfs-oppgradering.md)
    * ⚙️ **Proxmox VE 8** (Debian 12 / Bookworm) — legacy
      * [GPU Drivere](linux/debian/12/proxmox/8/gpu-drivere.md)
      * [NVIDIA Container Toolkit i Docker/LXC](linux/debian/12/proxmox/8/nvidia-container-toolkit-docker-lxc.md)

* 📝 **Bash / Scripting**
  * [Bash Scripting Guide](bash/bash-scripting-guide.md)

* 🤖 **AI / Ollama**
  * [Ollama Terminal](ai/ollama-terminal.md)
  * [Open WebUI på Proxmox LXC](ai/openwebui-proxmox-lxc.md)

* 🔒 **VPN**
  * [WireGuard (UniFi UCG Max)](vpn/wireguard-setup.md)
  * [NordVPN (UniFi)](vpn/nordvpn-unifi.md)

* 🌐 **Nettverk**
  * [VLAN Oppsett (UniFi)](network/vlan-setup.md)

* 🐳 **Docker**
  * [Dokumentserver Oppsett](docker/mkdocs-setup.md)

...
> **Oppdatert 27.08.2026:** skjermnavn og scale i eksemplene under er
> rettet til å matche bekreftet maskinvare (`hyprctl monitors`):
> ekstern skjerm er `HDMI-A-1` (Lenovo Legion 27Q-10), ikke `DP-1`,
> og begge skjermer kjører `scale 1` (ikke 1.20). Eksemplene her
> bruker fortsatt gammel `.conf`-syntaks — selve `hyprland.conf` er
> migrert til `hyprland.lua`, se `hyprland-lua-migrering.md`. Denne
> guiden er ikke konvertert til Lua ennå; workspace-scriptet under
> genererer fortsatt en `.conf`-fil som sources inn separat, og
> fungerer parallelt med `hyprland.lua` inntil videre. For selve
> monitor-oppsettet (skjerm + clamshell), se `hyprland-skjermoppsett.md`
> som er den autoritative, testede kilden.
...
```text
eHDMI-A-1 = intern laptopskjerm
HDMI-A-1  = ekstern Lenovo-skjerm
```

Aktuelle innstillinger:

```text
eHDMI-A-1: 1920x1200@60, scale 1
HDMI-A-1:  2560x1440@60, scale 1.00
```
...
```conf
monitor = eHDMI-A-1, 1920x1200@60, 0x0, 1
```
...
`1920x0` er logisk riktig fordi:

```text
1920 / 1 = 1920 (ingen omregning nødvendig med scale 1)
```

Dermed starter den eksterne skjermen rett etter laptopskjermens logiske bredde. (Kjører du en av skjermene med fraksjonell skalering, f.eks. 1.5, må denne verdien regnes i *logiske* piksler, ikke fysiske — se hyprland-skjermoppsett.md for et konkret eksempel på det.)
...
workspace = 11, monitor:eHDMI-A-1, persistent:true
workspace = 12, monitor:eHDMI-A-1, persistent:true
workspace = 13, monitor:eHDMI-A-1, persistent:true
workspace = 14, monitor:eHDMI-A-1, persistent:true
workspace = 15, monitor:eHDMI-A-1, persistent:true
workspace = 16, monitor:eHDMI-A-1, persistent:true
workspace = 17, monitor:eHDMI-A-1, persistent:true
workspace = 18, monitor:eHDMI-A-1, persistent:true
workspace = 19, monitor:eHDMI-A-1, persistent:true
workspace = 20, monitor:eHDMI-A-1, persistent:true
```
...
for i in {11..20}; do
    hyprctl dispatch moveworkspacetomonitor "$i" eHDMI-A-1
done
```
...
```bash
hyprctl dispatch focusmonitor eHDMI-A-1
